# coding: utf-8
'''
	WPB (4-12-2017): See change to class ObjCInstance, __new__ method
									 to alleviate memory corruption in photo.get_image_data
'''
...
class ObjCInstance (object):
	...
	def __new__(cls, ptr):
		...
		'''
			WPB (4/21/2017): 
				Changes to autorelease memory pool to alleviate memory leak 
				occuring in photos.get_image_data
				line changed from if ptr:
			'''
...
# Set a handler for uncaught ObjC exceptions that writes to a log file:
ExceptionHandlerFuncType = ctypes.CFUNCTYPE(None, c_void_p)
NSSetUncaughtExceptionHandler = c.NSSetUncaughtExceptionHandler
NSSetUncaughtExceptionHandler.restype = None
NSSetUncaughtExceptionHandler.argtypes = [ExceptionHandlerFuncType]
